...
public class UserServiceImpl implements UserService {
    ...
    public int updateUserPoint(Task task) {
        ...
        // user表中添加积分
        int row = userMapper.updateUserPoint(point, username);
        if(row <= 0){
            return 0;
        }
        PointLog newPointLog = new PointLog();
        newPointLog.setPoint(point);
        newPointLog.setUserId(username);
        newPointLog.setOrderId(orderId);
        int i = pointLogMapper.insertSelective(newPointLog);
        if(i <= 0){
            return 0;
        }
        // 释放redis的锁
        redisTemplate.delete(task.getId() + "");
        System.out.println("用户积分更新成功");
